...
class LLM:
    ...
    def chatCompletion(self,prompt="", user_context=None, masterPrompt="", initialAssistant="",model=None, maxTokens=1024):
        if model is None:
        ...
        try:
            response = self.litellm.chat.completions.create(model=model, messages=messages, max_tokens=maxTokens)
            return response
