...
def on_connect(client, userdata, rc, arg):
    ...
    debug("[GladosNode] Node name      : " + nodeName +
          " ,Global Shutdown : " + str(globalShutdown))
    ...
    publish("node/hello", "Hello! Im " + nodeName +
            ", Architecture : "+procType+" OS: "+osType)
    subscribeTopics()
    publishSystemInfo()
...
def publishSystemStats():
    try:
        mqttClient.publish("node/"+nodeName+"/system/cpuUsage",
                           psutil.cpu_percent(interval=1))
    except:
        pass
    try:
        mqttClient.publish("node/"+nodeName+"/system/temperatures",
                           str(psutil.sensors_temperatures()))
    except:
        pass
    try:
        mqttClient.publish("node/"+nodeName+"/system/diskUsage",
                           str(psutil.disk_usage('/')[3]))
    except:
        pass
