...
public:
	...
	void setup()
	{
		...
		setupNode();
#ifndef ESP32
	  //WiFi.begin("JarvisNetwork", "jointheovermind");storage.readConfig("bootError")
...
if(storage.readConfig("configMode") == "onReset")
{
	...
	void reconnect() {
		if(!m_reconnect) return;
		m_reconnectTimer -= m_lastCicleTime;
		if(m_reconnectTimer >0)
			return;
		Serial.println("Reconectando....");
		WiFi.begin(storage.readConfig("wifiESSID").c_str(), storage.readConfig("wifiPWD").c_str());
	...
	void imAlive()
	{
		Serial.print("Im alive! - ID:");Serial.print(m_nodeID);Serial.print("- IP:");Serial.println(localIP());Serial.print("- WifiMode:");Serial.println(WiFi.status());
